...
import { CryptoUtils, ImplicitAuthManager } from '../src/libs/ImplicitAuthManager';
...
describe('Implicit Auth Manager Class', () => {
  describe('Config Validations', () => {
    ...
    it('throws if redirectURI in config is not of type string', () => {
      ...
      expect(() => {
        const iam = new ImplicitAuthManager(config);
      }).toThrow('If passing in a custom redirectURI it must either be a function or a string');
    });
    ...
    it("throws if redirectURI in config is a function and doesn't return a string", () => {
      ...
      expect(() => {
        const iam = new ImplicitAuthManager(config);
      }).toThrow('If passing in a custom redirectURI as a function it must return a string');
    });
  ...
  describe('Instance helper methods', () => {
    ...
    test('getSSOLogoutURI returns a uri', () => {
      ...
      const iam = new ImplicitAuthManager(config);
      const expectedURI =
        iam.baseLogoutEndpoint + `?redirect_uri=mysite.com?intention=LOGOUT&sso=true`;
